...
window.U = (function () {
  ...
  const fmtDateLong = (d) => fmtDate(d, { day: 'numeric', month: 'long', year: 'numeric' });
  const fmtMonth = (d) => {
    const date = d instanceof Date ? d : new Date(d);
    return date.toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' });
  };
  const fmtEur = (n) => (Number(n) || 0).toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' });
  const fmtNum = (n, d = 2) => (Number(n) || 0).toLocaleString('fr-FR', { minimumFractionDigits: d, maximumFractionDigits: d });
  ...
  const colorFromString = (str) => {
    const palette = ['#2447df', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#ec4899', '#84cc16', '#f97316', '#0ea5e9'];
    let h = 0;
  ...
  const avatar = (name, size = 36) => {
    const bg = colorFromString(name || 'x');
    const fs = Math.round(size * 0.4);
    return `<span class="avatar" style="width:${size}px;height:${size}px;background:${bg};font-size:${fs}px;">${initials(name)}</span>`;
  };
  ...
  const toast = (msg, type = 'info') => {
    ...
    const icons = {
      success: '<svg class="w-5 h-5 text-emerald-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>',
      error:   '<svg class="w-5 h-5 text-red-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/></svg>',
      info:    '<svg class="w-5 h-5 text-brand-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/></svg>',
      warn:    '<svg class="w-5 h-5 text-amber-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.347 0-2.189-1.458-1.515-2.625L8.485 2.495zM10 5a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 5zm0 9a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"/></svg>',
    };
    el.innerHTML = `${icons[type] || icons.info}<div class="flex-1 text-slate-700">${escapeHtml(msg)}</div>`;
    document.getElementById('toast-container').appendChild(el);
    setTimeout(() => { el.style.opacity = '0'; el.style.transition = 'opacity .25s'; setTimeout(() => el.remove(), 250); }, 3000);
  };

  const confirm = (message, { title = 'Confirmation', confirmText = 'Confirmer', danger = false } = {}) =>
    new Promise((resolve) => {
      modal({
        title,
        body: `<p class="text-sm text-slate-600">${escapeHtml(message)}</p>`,
        footer: `
          <button class="btn btn-secondary" data-action="cancel">Annuler</button>
          <button class="btn ${danger ? 'btn-danger' : 'btn-primary'}" data-action="ok">${escapeHtml(confirmText)}</button>
        `,
        onMount: (root, close) => {
          root.querySelector('[data-action="cancel"]').onclick = () => { close(); resolve(false); };
          root.querySelector('[data-action="ok"]').onclick = () => { close(); resolve(true); };
        }
      });
    });

  let modalCloseFn = null;
  const modal = ({ title, body, footer = '', size = 'max-w-2xl', onMount }) => {
    ...
    `;
    const close = () => { backdrop.remove(); modalCloseFn = null; };
    modalCloseFn = close;
    backdrop.querySelector('[data-close]').onclick = close;
    backdrop.addEventListener('click', (e) => { if (e.target === backdrop) close(); });
    root.appendChild(backdrop);
    if (onMount) onMount(backdrop.querySelector('.modal-content'), close);
    return { close, root: backdrop.querySelector('.modal-content') };
  };
  const closeModal = () => modalCloseFn && modalCloseFn();
  ...
  const icons = {
    ...
    users:    '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m6-8a4 4 0 11-8 0 4 4 0 018 0zm6 3a3 3 0 11-6 0 3 3 0 016 0z"/></svg>',
    leave:    '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>',
    ...
    chart:    '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3"/></svg>',
    ...
    sitemap:  '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4m4-8h8m-8 16h8"/></svg>',
    ...
    trash:    '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6M1 7h22M9 7V4a1 1 0 011-1h4a1 1 0 011 1v3"/></svg>',
    ...
    onboard:  '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>',
    ...
    chevronR: '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>',
    chevronL: '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>',
    chevronD: '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>',
  ...
  return {
    uid, fmtDate, fmtDateLong, fmtMonth, fmtEur, fmtNum,
    today, addDays, daysBetween, businessDaysBetween,
    initials, colorFromString, avatar, escapeHtml,
    toast, modal, closeModal, confirm, debounce,
    downloadFile, csvFromRows, icons,
  };
})();
